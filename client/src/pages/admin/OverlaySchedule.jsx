...
function isScheduled(msg) { return !!(msg.start_at || msg.end_at); }
...
function StatusChip({ msg }) {
  ...
  if (isCurrentlyActive(msg))
    return <span className="text-xs px-2 py-0.5 rounded-full bg-green-500/20 text-green-400">Active now</span>;
  if (msg.start_at && new Date(msg.start_at).getTime() > Date.now())
    return <span className="text-xs px-2 py-0.5 rounded-full bg-blue-500/20 text-blue-400">Scheduled</span>;
  if (msg.end_at && new Date(msg.end_at).getTime() < Date.now())
    return <span className="text-xs px-2 py-0.5 rounded-full bg-orange-500/20 text-orange-400">Expired</span>;
    ...
    <div className="min-h-screen bg-tv-bg flex flex-col">
      ...
      <div className="bg-tv-accent border-b border-tv-borderSubtle px-6 py-4 flex-shrink-0">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <MobileMenu />
            <div>
              <h1 className="text-xl font-bold text-white">Overlay Scheduler</h1>
              <p className="text-xs text-white/70 hidden sm:block">Timed ticker messages for display bottom bars</p>
            </div>
          </div>
      ...
      <div className="flex-1 p-4 md:p-6 max-w-4xl mx-auto w-full pb-24 space-y-6">
        ...
        <div className="flex gap-2 flex-wrap">
          {['all', 'active', 'scheduled', 'disabled'].map(fil => (
            <button
              key={fil}
              onClick={() => setFilter(fil)}
              className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${filter === fil ? 'bg-tv-accent text-white' : 'bg-tv-bgSoft text-tv-textMuted hover:text-tv-text'}`}
            >
              {fil === 'all' ? `All (${messages.length})` : fil.charAt(0).toUpperCase() + fil.slice(1)}
            </button>
          ))}
        </div>
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            {groups.length === 0 && <p className="text-tv-textMuted text-sm text-center py-8">No messages match this filter.</p>}
            {groups.map(group => (
                ...
                <div className="space-y-2">
                  {group.items.map(msg => (
                    <div key={msg.id} className={`bg-tv-bgSoft border rounded-xl px-4 py-3 flex items-start gap-3 ${isCurrentlyActive(msg) ? 'border-green-500/30' : 'border-tv-borderSubtle'}`}>
                      <div className={`mt-1 text-base shrink-0 ${PRIORITY_COLORS[msg.priority] || 'text-tv-textMuted'}`}>■</div>
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 mt-1 flex-wrap text-xs text-tv-textMuted">
                          ...
                          {msg.target_type !== 'all' && <span>Target: {msg.target_type} #{msg.target_id}</span>}
                          {msg.start_at && <span>From: {fmtDT(msg.start_at)}</span>}
                          {msg.end_at   && <span>Until: {fmtDT(msg.end_at)}</span>}
                        </div>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        <button onClick={() => toggle(msg)} className={`text-xs px-2 py-1 rounded transition-colors ${msg.enabled ? 'bg-yellow-500/20 text-yellow-400 hover:bg-yellow-500/30' : 'bg-green-500/20 text-green-400 hover:bg-green-500/30'}`}>
      ...
      {/* Create/Edit modal — always rendered, controlled via isOpen */}
      <Modal isOpen={showModal} title={editing ? 'Edit Message' : 'New Overlay Message'} onClose={() => setShowModal(false)}>
        <div className="space-y-4">
          ...
          <div>
            <p className="text-xs font-medium text-tv-textMuted mb-2">Schedule Window (optional — blank = always on)</p>
            <div className="grid grid-cols-2 gap-3">
              <div>
